<template>
  <div class="flex flex-col items-center justify-center gap-4 w-full">
    <div class="relative flex items-center justify-center p-4 w-full overflow-auto">
      <div class="project-explorer-preview-wrapper" style="width: 100%; max-width: 600px; margin: 0 auto;">
        <VueFlow
          :nodes="[demoNode]"
          :edges="[]"
          ...
          class="docs-node-preview-flow project-explorer-flow"
        >
          <template #node-projectExplorer="{ data: node }">
    ...
    <p class="text-sm text-muted-foreground text-center max-w-md">
      Project structure explorer for browsing and navigating project files and folders.
    </p>
  </div>
</template>
...
const ProjectExplorerNode = defineAsyncComponent(() => import("~/components/canvas/nodes/childNodes/ProjectExplorerNode.vue"));

const nodeTypes = {
  projectExplorer: ProjectExplorerNode
};

const demoNode = ref({
  id: "demo-project-explorer-node",
  type: "projectExplorer",
  position: { x: 0, y: 0 },
  selected: true,
  data: {
    label: "Project Explorer",
    projectPath: "/sample/project/path"
  }
});
</script>
...
.docs-node-preview-flow.project-explorer-flow {
...
.project-explorer-flow :deep(.project-explorer-node-container) {
  width: 100% !important;
  max-width: 600px !important;
