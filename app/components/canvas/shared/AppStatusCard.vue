<template>
	<div class="app-status-card p-3 border border-white/10 rounded-lg bg-black/20">
		<div class="flex items-center gap-3">
			...
			<div class="ml-auto flex gap-2">
				<button
					...
					class="px-3 py-1.5 bg-green-500/20 hover:bg-green-500/30 text-green-400 rounded-lg text-sm border border-green-500/30 transition-all duration-200 hover:scale-105"
...
<script setup lang="ts">
	import type { AppConfig } from "../composables/useAppDetection";
	...
	const emit = defineEmits<{
		"app-detected": [data: { installed: boolean; version?: string }]
		"app-installed": [data: { success: boolean }]
		"app-opened": [data: { success: boolean }]
		"app-error": [data: { error: string }]
	}>();

	const { status, detectApp, openApp, installApp, openInstallationGuide, initializeApp } = useAppDetection(props.config);
	const showInstallGuide = ref(props.showInstallGuide);

	// Auto-check on mount if enabled
	watch(() => props.autoCheck, (autoCheck) => {
		if (autoCheck) {
			initializeApp();
		}
	}, { immediate: true });
	...
	watch(() => status.value.installed, (installed) => {
		if (installed) {
			emit("app-detected", {
				installed: true,
	...
	const handleCheckClick = async () => {
		try {
			await detectApp(true);
			emit("app-detected", {
				installed: status.value.installed,
				version: status.value.version
			});
		} catch (error: any) {
			emit("app-error", { error: error.message || "Failed to check app" });
		}
	};
	...
	const handleInstallClick = async () => {
		try {
			...
			emit("app-installed", { success: result.success });
		} catch (error: any) {
			emit("app-error", { error: error.message || "Failed to install app" });
		}
	};
	...
	const handleOpenClick = async () => {
		try {
			await openApp();
			emit("app-opened", { success: true });
		} catch (error: any) {
			emit("app-error", { error: error.message || "Failed to open app" });
		}
	};
