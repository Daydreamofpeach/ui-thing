  ...
  <div class="flex flex-col items-center justify-center gap-4 w-full">
    <div class="relative flex items-center justify-center p-4 w-full overflow-auto">
      <div class="node-check-preview-wrapper" style="width: 100%; max-width: 450px; margin: 0 auto;">
        ...
        >
          <template #node-cursorCheckNode="{ data: node }">
    ...
    <p class="text-sm text-muted-foreground text-center max-w-md">
      Cursor checker node for detecting and verifying Cursor installation.
    </p>
  </div>
</template>
...
const CursorCheckNode = defineAsyncComponent(() => import("~/components/canvas/nodes/CursorCheckNode.vue"));

const nodeTypes = {
  cursorCheckNode: CursorCheckNode
};

const demoNode = ref({
  id: "demo-cursor-check-node",
  type: "cursorCheckNode",
  position: { x: 0, y: 0 },
  selected: true,
  data: {
    label: "Cursor Check"
