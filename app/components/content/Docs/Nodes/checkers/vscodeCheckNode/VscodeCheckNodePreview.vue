  ...
  <div class="flex flex-col items-center justify-center gap-4 w-full">
    <div class="relative flex items-center justify-center p-4 w-full overflow-auto">
      <div class="node-check-preview-wrapper" style="width: 100%; max-width: 450px; margin: 0 auto;">
        ...
        >
          <template #node-vscodeCheckNode="{ data: node }">
    ...
    <p class="text-sm text-muted-foreground text-center max-w-md">
      VS Code checker node for detecting and verifying Visual Studio Code installation.
    </p>
  </div>
</template>
...
const VscodeCheckNode = defineAsyncComponent(() => import("~/components/canvas/nodes/VscodeCheckNode.vue"));

const nodeTypes = {
  vscodeCheckNode: VscodeCheckNode
};

const demoNode = ref({
  id: "demo-vscode-check-node",
  type: "vscodeCheckNode",
  position: { x: 0, y: 0 },
  selected: true,
  data: {
    label: "VS Code Check"
