    ...
    <div class="w-full md:w-1/2">
      <div class="mx-auto w-full max-w-[330px] px-5">
        ...
        <p class="mt-8 text-sm text-muted-foreground">
          ...
          <NuxtLink class="font-semibold text-primary underline-offset-2 hover:underline" to="/account/login"
            >Log in</NuxtLink
          >
        </p>
      </div>
...
<script lang="ts" setup>
  ...
  const submit = handleSubmit(async (values) => {
    try {
      const response = await $fetch<{ success: boolean; user: any; message: string }>('/api/auth/signup', {
        method: 'POST',
        body: {
          name: values.name,
          email: values.email,
