...
onMounted(() => {
  const minimalDoc = "WebStorm checker node using AppStatusCard component and useAppDetection composable for reliable detection and installation support.";
  ...
  const mainCode = `<template>
	<BaseNodeTemplate
		...
		icon-image="/icons/apps/webstorm.svg"
		:title="_props.customNodeProps.data?.label || 'WEBSTORM CHECK'"
		...
		:node-class="webstormCheckNodeClass"
		@close="handleClose"
	>
		<div class="node-content-wrapper">
			<AppStatusCard
				:config="webstormConfig"
				:auto-check="true"
...
<script setup lang="ts">
	...
	interface Props {
		customNodeProps: {
			...
			data: {
				label?: string
				webstormInstalled?: boolean
				webstormVersion?: string
			}
			selected?: boolean
		}
	...
	const webstormConfig = appConfigs.webstorm;
	const { status } = useAppDetection(webstormConfig);
	...
	const webstormCheckNodeClass = computed(() => {
		return \`webstorm-check-node \${status.value.installed ? 'webstorm-found' : ''}\`;
	});
	...
	const handleAppDetected = (data: any) => {
		emit("detectWebstorm", data);
	};

	const handleAppInstalled = (data: any) => {
		emit("installWebstorm", data);
	};

	const handleAppError = (data: any) => {
		emit("webstormError", data);
	};
<\/script>
...
.webstorm-check-node.webstorm-found :deep(.base-node-template) {
	border-color: rgba(34, 197, 94, 0.8);
	animation: webstormPulse 2s ease-in-out infinite;
	box-shadow: 0 0 20px rgba(34, 197, 94, 0.3);
}

@keyframes webstormPulse {
	0%, 100% {
		border-color: rgba(34, 197, 94, 0.8);
		box-shadow: 0 0 20px rgba(34, 197, 94, 0.3);
	}
  ...
  const files: FileStructure[] = [
    {
      title: "WebstormCheckNode.vue",
      icon: "vscode-icons:file-type-vue",
      path: "components/canvas/nodes/WebstormCheckNode.vue",
      content: mainCode
    },
    {
