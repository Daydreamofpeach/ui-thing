<template>
  <div class="flex flex-col items-center justify-center gap-4 w-full">
    <div class="relative flex items-center justify-center p-4 w-full overflow-auto">
      <div class="dotnet-check-preview-wrapper" style="width: 100%; max-width: 450px; margin: 0 auto;">
        <VueFlow
          :nodes="[demoNode]"
          :edges="[]"
        ...
        >
          <template #node-dotnetCheckNode="{ data: node }">
    ...
    <p class="text-sm text-muted-foreground text-center max-w-md">
      .NET version checker node for detecting and verifying .NET SDK installation.
    </p>
  </div>
</template>
...
const DotnetCheckNode = defineAsyncComponent(() => import("~/components/canvas/nodes/DotNetCheckNode.vue"));
...
const demoNode = ref({
  id: "demo-dotnet-check-node",
  type: "dotnetCheckNode",
  position: { x: 0, y: 0 },
  selected: true,
  data: {
    dotnetVersion: "8.0.0"
