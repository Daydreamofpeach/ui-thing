...
onMounted(() => {
  const minimalDoc = "Sublime Text checker node using AppStatusCard component and useAppDetection composable for reliable detection and installation support.";
  ...
  const mainCode = `<template>
	<BaseNodeTemplate
		...
		icon-image="/icons/apps/sublime.svg"
		:title="_props.customNodeProps.data?.label || 'SUBLIME CHECK'"
		...
		:node-class="sublimeCheckNodeClass"
		@close="handleClose"
	>
...
<script setup lang="ts">
	...
	const sublimeConfig = appConfigs.sublime;
	const { status } = useAppDetection(sublimeConfig);
	...
	const sublimeCheckNodeClass = computed(() => {
		return \`sublime-check-node \${status.value.installed ? 'sublime-found' : ''}\`;
	});
	...
	const handleAppDetected = (data: any) => {
		emit("detectSublime", data);
	};

	const handleAppInstalled = (data: any) => {
		emit("installSublime", data);
	};

	const handleAppError = (data: any) => {
		emit("sublimeError", data);
	};
<\/script>
...
.sublime-check-node.sublime-found :deep(.base-node-template) {
	border-color: rgba(34, 197, 94, 0.8);
	animation: sublimePulse 2s ease-in-out infinite;
...
@keyframes sublimePulse {
  ...
  const files: FileStructure[] = [
    {
      title: "SublimeCheckNode.vue",
      icon: "vscode-icons:file-type-vue",
      path: "components/canvas/nodes/SublimeCheckNode.vue",
      content: mainCode
    },
    {
