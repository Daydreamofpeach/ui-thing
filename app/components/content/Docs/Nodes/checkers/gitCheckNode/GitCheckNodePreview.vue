  ...
  <div class="flex flex-col items-center justify-center gap-4 w-full">
    <div class="relative flex items-center justify-center p-4 w-full overflow-auto">
      <div class="node-check-preview-wrapper" style="width: 100%; max-width: 450px; margin: 0 auto;">
        ...
        >
          <template #node-gitCheckNode="{ data: node }">
    ...
    <p class="text-sm text-muted-foreground text-center max-w-md">
      Git checker node for detecting and verifying Git installation.
    </p>
  </div>
</template>
...
const GitCheckNode = defineAsyncComponent(() => import("~/components/canvas/nodes/GitCheckNode.vue"));

const nodeTypes = {
  gitCheckNode: GitCheckNode
};

const demoNode = ref({
  id: "demo-git-check-node",
  type: "gitCheckNode",
  position: { x: 0, y: 0 },
  selected: true,
  data: {
